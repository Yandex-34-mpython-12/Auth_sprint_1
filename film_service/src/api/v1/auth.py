...
def decode_token(token: str) -> Optional[dict]:
    try:
        decoded_token = jwt.decode(token, settings.jwt_secret_key, audience="fastapi-users:auth", algorithms=[settings.jwt_algorithm])
...
def role_required(role: str):
    def role_checker(user: Annotated[dict, Depends(security_jwt)]):
