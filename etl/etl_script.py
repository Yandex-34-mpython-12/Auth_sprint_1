...
def postgres_connection(**connection_params):
    pg_conn = psycopg2.connect(**connection_params, cursor_factory=psycopg2.extras.DictCursor)
    try:
        yield pg_conn
    finally:
        pg_conn.close()
...
def import_data_to_es(bulk_data, index):
    ...
    if response['errors']:
        raise ValueError(f"Error inserting data into Elasticsearch: {response}")
    else:
...
def transfer_filmworks(storage):
    last_processed_timestamp_filmworks = storage.get_state('last_processed_timestamp_filmworks')
    logger.info(f"Update ES {FILMWORKS_INDEX} with new content from: {last_processed_timestamp_filmworks}")
    data = fetch_data_from_postgres(filmworks_query, last_processed_timestamp_filmworks)
    bulk_data = prepare_filmworks(data)
    import_data_to_es(bulk_data, FILMWORKS_INDEX)
    storage.set_state('last_processed_timestamp_filmworks', datetime.now().isoformat())


def transfer_genres(storage):
    last_processed_timestamp_genres = storage.get_state('last_processed_timestamp_genres')
    logger.info(f"Update ES {GENRES_INDEX} with new content from: {last_processed_timestamp_genres}")
    data = fetch_data_from_postgres(genres_query, last_processed_timestamp_genres)
    bulk_data = prepare_genres(data)
    import_data_to_es(bulk_data, GENRES_INDEX)
    storage.set_state('last_processed_timestamp_genres', datetime.now().isoformat())


def transfer_persons(storage):
    last_processed_timestamp_persons = storage.get_state('last_processed_timestamp_persons')
    logger.info(f"Update ES {PERSONS_INDEX} with new content from: {last_processed_timestamp_persons}")
    data = fetch_data_from_postgres(persons_query, last_processed_timestamp_persons)
    bulk_data = prepare_persons(data)
    import_data_to_es(bulk_data, PERSONS_INDEX)
    storage.set_state('last_processed_timestamp_persons', datetime.now().isoformat())
